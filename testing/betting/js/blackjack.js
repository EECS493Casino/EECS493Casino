...
function newgame() {
	...
	$("#totalFunds").html("$" + (playerFunds - currentBet));
	$("#bettingSlider").slider({ disabled: true });
}
function endGame() {
    ...
    if (allPlayers.getPlayer(1).score() > 21)
        ...
        if (allPlayers.getPlayer(0).score() > 21) {
            ...
            newTotalFunds = playerFunds + currentBet;
    ...
    playerFunds = newTotalFunds;
    $("#totalFunds").html("$" + newTotalFunds);
    document.getElementById("output").innerHTML = "<p>" + outputtext + "</p><button class='btn active' onclick='newgame()'>Another Round?</button>";
    $("#bettingSlider").slider({ disabled: false });
}
